...
static int luacore_canonicalize(lua_State *L) {
    ...
    char *file = (char *)xmalloc(L, filelen + 1);
    memcpy(file, input, filelen);
    file[filelen] = '\0';

    if (file[0] != '/') {
        size_t size = 1024;
        char *cwdbuf = (char *)xmalloc(L, size);
        char *retbuf = NULL;
        while ((retbuf = getcwd(cwdbuf, size)) == NULL && errno == ERANGE) {
            size += 32;
            cwdbuf = (char *)xrealloc(L, cwdbuf, size);
            errno = 0;
        }
        ...
        if (size < filelen + retbuflen) {
            retbuf = (char *)xrealloc(L, retbuf, filelen + retbuflen + 2);
        }
        retbuf[retbuflen] = '/';
    ...
    char *pathbuf = (char *)xmalloc(L, pathbufsize);
    size_t pathlen = 0;
    size_t retbufsize = 1024;
    char *retbuf = (char *)xmalloc(L, retbufsize);
    ...
    while (*start != '\0') {
        ...
        if (pathbufsize <= pathlen + componentlen) {
            ...
            pathbuf = (char *)xrealloc(L, pathbuf, pathbufsize);
        }
        ...
        while (n >= (ssize_t)retbufsize) {
            ...
            retbuf = (char *)xrealloc(L, retbuf, retbufsize);
        ...
        if (n >= 0L) {
            ...
            if (retbuf[0] == '/') {
                ...
                if ((size_t)n != pathlen) {
                    ...
                    file = (char *)xrealloc(L, file, filelen + 1);
